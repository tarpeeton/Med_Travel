...
const Sanathory: FC<ISanathoryData> = ({ data, cotegory }) => {
  ...
  useEffect(() => {
    // Filter the data whenever the active button changes
    if (activeButton) {
      setFilteredData(data.filter(item => item.goal?.name === activeButton))
    }
  }, [activeButton, data]) // Update when activeButton or data changes
      ...
      <div>
        ...
        <div className='hidden mt-[30px] mdl:flex mdl:flex-row mdl:flex-wrap mdl:gap-[8px]'>
          {cotegory.map((category, index) => (
            <button
              key={index}
              className={`py-[12px] px-[20px] text-center  font-semibold  flex items-center justify-center rounded-full font-raleway text-[15px]  ${getButtonStyle(category.name)}`}
              onClick={() => handleButtonClick(category.name)} // Handle button click
            >
              {category.name}
            </button>
          ))}
              ...
              filteredData.slice(0, sliceNumber).map((t, index) => (
                <div key={index} className='mdl:w-[48%] 2xl:w-[32%] cursor-pointer group 2xl:mb-[50px]'>
                  ...
                  <div className='mt-[12px] mdl:mt-[16px] 2xl:mt-[20px] flex flex-col'>
                    ...
                    <div>
                      <p className='text-[14px] mdl:text-[17px] 2xl:text-[18px] font-raleway font-medium text-titleDark'>{t.address}</p>
                    </div>

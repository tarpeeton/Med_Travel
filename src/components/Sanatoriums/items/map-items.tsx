...
interface ISanathoryMapItemProps {
...
const SanathoryMapItem: FC<ISanathoryMapItemProps> = ({address, phone, telegram, instagram , }) => {
    ...
    const initializeMap = () => {
      if (window.ymaps && address?.coords?.[0]) {
        ...
        if (coords.length === 2 && !isNaN(coords[0]) && !isNaN(coords[1])) {
          ...
          // Создаём карту
          const map = new window.ymaps.Map('mapSanathoryItem', {
      ...
      <div className="flex flex-col">
        ...
        <div className="rounded-[30px] mt-[20px] overflow-hidden w-full mdl:mt-[40px]">
          <div className='h-[300px] mdl:h-[400px] 2xl:h-[500px]'>
            <div id="mapSanathoryItem" className="w-full h-full" />
          </div>
        </div>
      </div>
    </div>
...
export default SanathoryMapItem

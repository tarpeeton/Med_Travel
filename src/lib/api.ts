...
export const consultation = async (
  ...
  try {
    ...
    const response = await axios.post(
      `${BASE_URL}/api/application/consultation`,
      {
        name,
        phone,
        email
...
export const AllSanathoriums = async (
  acceptLanguage: string = 'en',
  options: IAllSanathorimFilter = {}
) => {
  try {
    // Destructure and set default values for parameters
    const {
...
export const BlogSlug = async (acceptLanguage: string = 'ru', slug: string) => {
  try {
...
export const AllBlogs = async (acceptLanguage: string = 'ru') => {
  try {
...
export const CreateCommentWithID = async (blogID: number, comment: string) => {
  try {
    const response = await axios.post(`${BASE_URL}api/blog/comment/${blogID}`, {
      comment
    })
